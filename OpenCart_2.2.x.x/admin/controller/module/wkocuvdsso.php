...
class ControllerModuleWkocuvdsso extends Controller {
	...
	public function index() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('module/wkocuvdsso', 'token=' . $this->session->data['token'], true)
		);
		...
		if ($data['apps']) {
			foreach ($data['apps'] as $key => $app) {
				$data['apps'][$key]['edit'] = $this->url->link('module/wkocuvdsso/add', 'token=' . $this->session->data['token']."&id=".$app['id'], true);
			}
		}

		$data['token'] = $this->session->data['token'];

		$data['add'] = $this->url->link('module/wkocuvdsso/add', 'token=' . $this->session->data['token'], true);

		$data['delete'] = $this->url->link('module/wkocuvdsso/delete', '' , true);

		$data['action'] = $this->url->link('module/wkocuvdsso', 'token=' . $this->session->data['token'], true);
	...
	public function add(){
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateAdd()) {
			...
			$this->response->redirect($this->url->link('module/wkocuvdsso', 'token=' . $this->session->data['token'], true));
		}
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('module/wkocuvdsso/add', 'token=' . $this->session->data['token'], true)
		);

		if (isset($this->request->get['id']) && $this->request->get['id']) {
			$data['action'] = $this->url->link('module/wkocuvdsso/add', 'token=' . $this->session->data['token'] . '&id=' . $this->request->get['id'], true);
		} else {
			$data['action'] = $this->url->link('module/wkocuvdsso/add', 'token=' . $this->session->data['token'], true);
		}

		$data['cancel'] = $this->url->link('module/wkocuvdsso', 'token=' . $this->session->data['token'], true);
