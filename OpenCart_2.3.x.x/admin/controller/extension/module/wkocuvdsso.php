...
class ControllerExtensionModuleWkocuvdsso extends Controller {
	...
	public function index() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('extension/module/wkocuvdsso', 'token=' . $this->session->data['token'], true)
		);
		...
		if ($data['apps']) {
			foreach ($data['apps'] as $key => $app) {
				$data['apps'][$key]['edit'] = $this->url->link('extension/module/wkocuvdsso/add', 'token=' . $this->session->data['token']."&id=".$app['id'], true);
			}
		}

		$data['token'] = $this->session->data['token'];

		$data['add'] = $this->url->link('extension/module/wkocuvdsso/add', 'token=' . $this->session->data['token'], true);

		$data['delete'] = $this->url->link('extension/module/wkocuvdsso/delete', '' , true);

		$data['action'] = $this->url->link('extension/module/wkocuvdsso', 'token=' . $this->session->data['token'], true);
		...
		$this->response->setOutput($this->load->view('extension/module/wkocuvdsso.tpl', $data));
	}
	...
	public function add(){
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateAdd()) {
			...
			$this->response->redirect($this->url->link('extension/module/wkocuvdsso', 'token=' . $this->session->data['token'], true));
		}
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('extension/module/wkocuvdsso/add', 'token=' . $this->session->data['token'], true)
		);

		if (isset($this->request->get['id']) && $this->request->get['id']) {
			$data['action'] = $this->url->link('extension/module/wkocuvdsso/add', 'token=' . $this->session->data['token'] . '&id=' . $this->request->get['id'], true);
		} else {
			$data['action'] = $this->url->link('extension/module/wkocuvdsso/add', 'token=' . $this->session->data['token'], true);
		}

		$data['cancel'] = $this->url->link('extension/module/wkocuvdsso', 'token=' . $this->session->data['token'], true);

		$data['header'] = $this->load->controller('common/header');
		$data['column_left'] = $this->load->controller('common/column_left');
		$data['footer'] = $this->load->controller('common/footer');

		$this->response->setOutput($this->load->view('extension/module/wkocuvdsso_form.tpl', $data));
	}
